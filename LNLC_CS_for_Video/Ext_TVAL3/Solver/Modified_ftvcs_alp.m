function [U, delta,gamma,out] = Modified_ftvcs_alp(A,b,p,q,opts,XX,uu,sita,alpha,gamma,flag)
gamma=gamma(:);
alpha=alpha(:);
%新增参数delta,XX,uu,sita,alpha,gamma,//alpha取代原始程序中的delta
%%%注意初始化U!带进来U=XX;
%实际修改代码采用输入α^((k) )代替delta,注意delta和delta’的区别


%L(x)=TV(x_k )-α^((k) ) (y-Φx)+β/2 ‖y-Φx‖_2^2-γ^((k)) (x-u^((k) ) )+θ/2||x-u^((k)) ||_2^2
...
% 
%这是原始的： min_{u,w} sum ||w_i|| - sigma'(Du-w) - delta'(Au-b) 
%                   + beta/2 ||Du-w||_2^2 + mu/2||Au-b||_2^2 ,
%

%现在的
% min_{u,w} sum ||w_i|| - sigma'(Du-w) + beta/2 ||Du-w||_2^2 - α^((k) ) (Au-b) 
%                    + mu/2||Au-b||_2^2 -γ^((k)) (x-u^((k) ) )+θ/2||x-u^((k)) ||_2^2,

%修改的函数只用更新sigma了，只有一个x(u)=w了

%只需把α^((k) ),γ^((k)),u^((k) ),θ的值作为参数带进TV，并多计算lam6 =-γ^((k)) (x-u^((k) ) )+θ/2||x-u^((k)) ||_2^2即可，原来的delta不需要计算了。
...
%增加　lam6 =-γ^((k)) (x-u^((k) ) )+θ/2||x-u^((k)) ||_2^2
%   f  = lam1 + beta/2 lam2 + mu/2 lam3 - lam4 - lam5
%   变为 f=lam1 + beta/2 lam2 + mu/2 lam3 - lam4 - lam5+lam6
...
%[U,mu,beta] = ftvcs_al_init(p,q,Atb,scl,opts);    % U: p*q  %初始化这个U的过程不要了,直接输入，但要计算这里mu和beta
...
sigmay = zeros(p,q);
%delta=alpha;  %%%alpha从外部带入,输入与否好象差别不大
%  delta = zeros(length(b),1);                % delta: m，改为输入了alpha
 if flag==0
      delta = zeros(length(b),1);                % delta: m，改为输入了alpha
    % gamma = zeros(length(uu(:)),1); %2018/9/15，不能初始化，性能急降
 else
     delta=alpha;  %%%alpha从外部带入,输入与否好象差别不大
 end;
...
out.lam1 = []; out.lam2 = []; out.lam3 = []; out.lam4 = []; out.lam5 = [];out.lam6 = [];%增加了lam6
out.itr = Inf;
...
[lam2,lam3,lam4,lam5,lam6,f,g2,Au,g] = get_g(U,Ux,Uy,Wx,Wy,lam1,beta,mu,A,b,...
    Atb,sigmax,sigmay,delta,uu,sita,gamma);%%增加了lam6,必然要增加uu和gamma,sita
%lam, f: constant      g2: pq        Au: m         g: pq

% compute gradient
d = g2 + muDbeta*g - DtsAtd;
d = d + sita*(U(:)-uu(:)) - gamma;%新增加的两项　　-γ^((k) ) x+θ(x-u^((k)))

count = 1;
Q = 1; C = f;                     % Q, C: costant
out.f = [out.f; f]; out.C = [out.C; C];
out.lam1 = [out.lam1; lam1]; out.lam2 = [out.lam2; lam2]; out.lam3 = [out.lam3; lam3];
out.lam4 = [out.lam4; lam4]; out.lam5 = [out.lam5; lam5];out.lam5 = [out.lam6; lam6];
...
for ii = 1:opts.maxit
    ...
    [lam2,lam3,lam4,lam5,lam6,f,g2,Au,g] = get_g(U,Ux,Uy,Wx,Wy,lam1,beta,mu,A,b,...
        Atb,sigmax,sigmay,delta,uu,sita,gamma);
    ...
    while f > C - alpha*const
        if cnt == 5
            ...
            [lam2,lam3,lam4,lam5,lam6,f,g2,Au,g] = get_g(U,Ux,Uy,Wx,Wy,lam1,...
                beta,mu,A,b,Atb,sigmax,sigmay,delta,uu,sita,gamma);
        ...
        [U,lam2,lam3,lam4,lam5,lam6,f,Ux,Uy,Au,g,g2] = update_g(p,q,lam1,...
            alpha,beta,mu,Up,du,gp,dg,g2p,dg2,Aup,dAu,Wx,Wy,Uxp,dUx,...
            Uyp,dUy,b,sigmax,sigmay,delta,uu,sita,gamma);
        cnt = cnt + 1;
    end
...
%             fprintf('Number of total iterations is %d. \n',out.itr);
        ...
        % update multipliers
        [sigmax,sigmay,delta,lam4,lam5,lam6,f] = update_mlp(beta,mu, ...
            Wx,Wy,Ux,Uy,Au,b,sigmax,sigmay,delta,lam4,lam5,lam6,f,U,uu,sita,gamma);
        ...
        f = lam1 + beta/2*lam2 + mu/2*lam3 - lam4 - lam5 + lam6;%增加了lam6
...
function [lam2,lam3,lam4,lam5,lam6,f,g2,Au,g] = get_g(U,Ux,Uy,Wx,Wy,lam1,...
    beta,mu,A,b,Atb,sigmax,sigmay,delta,uu,sita,gamma)
global Dt
...
%新增的计算lam6 =-γ^((k)) (x-u^((k) ) )+θ/2||x-u^((k)) ||_2^2
lam6=-gamma'*(U(:)-uu(:))+sita/2*norm((U(:)-uu(:)),'fro')^2;
% f
f = lam1 + beta/2*lam2 + mu/2*lam3 - lam4 - lam5 + lam6;




function [U,lam2,lam3,lam4,lam5,lam6,f,Ux,Uy,Au,g,g2] = update_g(p,q,lam1,...
    alpha,beta,mu,Up,du,gp,dg,g2p,dg2,Aup,dAu,Wx,Wy,Uxp,dUx,Uyp,dUy,b,...
    sigmax,sigmay,delta,uu,sita,gamma)
...
function [sigmax,sigmay,delta,lam4,lam5,lam6,f] = update_mlp(beta,mu, ...
    Wx,Wy,Ux,Uy,Au,b,sigmax,sigmay,delta,lam4,lam5,lam6,f,U,uu,sita,gamma)
...
Aub = Au-b;
delta = delta - mu*Aub;%改为输入了alpha
 %gamma = gamma - sita*(U(:)-uu(:));% gamma 的更新在外面,更新会稍好

tmpf = f + lam4 + lam5 - lam6;
lam4 = sum(sum(conj(sigmax).*Vx + conj(sigmay).*Vy));
lam5 = delta'*Aub;
%新增的计算lam6 =-γ^((k)) (x-u^((k) ) )+θ/2||x-u^((k)) ||_2^2
lam6=-gamma'*(U(:)-uu(:))+sita/2*norm((U(:)-uu(:)),'fro')^2;

f = tmpf - lam4 - lam5 + lam6;
